...
async function renderEpisodes() {
  ...
  <div class="episode" data-url="${ep.videoUrl}" onclick="playEpisode('${ep.videoUrl}')" tabindex="0">
    ...
    <div class="episode-actions">
      <div class="material-icons download-icon" onclick="event.stopPropagation(); handleDownload('${ep.downloadUrl}', '${ep.title.replace(/'/g, "")}')" tabindex="0">arrow_downward</div>
...
function handleDownload(url, title) {
  ...
  // 🔴 LINKS MONETIZADOS → abrir fuera (navegador)
  if (url.includes("earnvids.com") || url.includes("dintezuvio.com")) {
    window.open(url, "_blank");
    return;
  }

  // 🟢 DESCARGA DIRECTA → Android nativo (con nombre del episodio)
  if (url.includes("od.lk") && window.Android && Android.downloadFile) {
    Android.downloadFile(url, title);
    return;
  }
